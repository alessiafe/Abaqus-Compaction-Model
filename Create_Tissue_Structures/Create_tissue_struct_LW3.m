...
imname = 'testspruce_LW3'; % image name
...
stats([25,48,119]) = []; % remove fibers for LW3
        ...
        if ~isempty(xi)
            ...
            for rep=1:2
                ...
                if rowidx1~=rowidx2
                    ...
                    % adjust intersection for LW3
                    if (i==21 && k==20 && rep==1); idx = [1, idx(1)+1, idx, idx(end)-1]; end
                    if (i==84 && k==82 && rep==2); idx = [idx(1)-1, idx, idx(end)+1]; end
                    if (i==84 && k==82 && rep==1); idx = [1, idx(1)+1, idx, idx(end)-1]; end
...
for i = 1:length(stats)
    ...
    stats(i).wthick = stats(i).wthick *downscale;
...
for i=1:2
    ...
    m = 0; % slope
    if i == 1
        maskpts(2,idx(:)) = m*maskpts(1,idx(:))+b(i)+4*layer_thick(1); % replace y
    else
        maskpts(2,idx(:)) = m*maskpts(1,idx(:))+b(i);
    end
end
...
for i=1:length(stats)
    % Store data
    mstruct.fibers(i).outerpts = stats(i).outerpts;
    mstruct.fibers(i).innerpts = stats(i).innerpts;  
    mstruct.fibers(i).Centroid = stats(i).Centroid;
    mstruct.fibers(i).wthick = stats(i).wthick; 
    mstruct.fibers(i).lthicks = stats(i).lthick;
end
