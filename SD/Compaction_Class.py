...
class Compaction_Class:
        ...
        if bcValue>=0 and bcValue<=1: # check if the compaction level lies between 0 and 1 (included)
            ...
            if RPnode:
                ...
                myModel.DisplacementBC(amplitude=ampName, createStepName=stepName, distributionType=UNIFORM,
                    fieldName='', fixed=OFF, localCsys=None, name=bcondName, region=myRegion, u1=disp_nodes[0], u2=disp_nodes[1]) # create displacement boundary condition
    ...
    # - numIntervals = number of intervals for output request (=20 by default)
        ...
        #myModel.EquallySpacedAmplitude(name=ampName, timeSpan=STEP, fixedInterval=1, data=(0., temperatureVar))
    ...
    # 
        ...
        if test_type in ['MD', 'VD', 'SD']:
            ...
            if test_type == 'SD':
                self.addTemperature = True
                dT_match = search(r'dT(\d+)', jobName)
                temp = int(dT_match.group(1))
    ...
    # Save results 
        ...
        for inc in range(numInc):
            ...
            if self.addRPsRequest:
                ...
                disp2[inc] = myU2[1]
                edge2[inc] = disp1[inc]+tissueBox[3]
            ...
            if self.addTemperature:
                ksi = tcurrent[inc]/t1
                temperature[inc] = comp_info['max_temperature']* pow(ksi,3) * (10 - 15*ksi + 6*pow(ksi,2))
