# Simulation of Mechanical Densification (MD)

This folder contains all scripts and files used for the **simulation of mechanical densification** of wood in the radial direction.  
The phenomenon is modeled by applying opposite quasi-static vertical displacements to reference points kinematically coupled to the top and bottom plates in contact with the tissue structure.
...
- **`MD_simulator.py`**  
  ...
  - *degree of compaction* as maximum fraction (from 0 to 1) of the tissue’s total radial size to be compacted.

- **`run_simulation_from_terminal.py`**  
  Script to **run the Abaqus simulation (`MD_simulator.py`) directly from the terminal**.
...
The names of the result files (`.odb`, `.npz`, `.csv`) indicate the tissue type, the densification protocol, and the corresponding moisture content and lignin in percentage.
