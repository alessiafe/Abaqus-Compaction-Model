...
# Set paths
mainpath = os.path.abspath('calculate_3D_structure_stiffness.py')
...
minSizeFiber = 0.3 # fiber mesh size factor
meshSizeMatrix = 1. # matrix mesh global size
minSizeMatrix = 1. # matrix mesh size factor
...
for k in range(len(struct_arr)):
    ...
    #l_thick = 1. # longitudinal thickness [microm]
    #modelName = compaction.setup_geometry(filepath, l_thick) 
    ...
    for i in range(len(moisture_arr)):
        for j in range(len(ligred_arr)):
            ...
            compaction.create_surface_partition(0.5, sides=True) # create surfaces to apply boundary conditions
            ...
            compaction.compute_3D_stiffness_matrix(eps, stepName, jobName[-1], savepath)
