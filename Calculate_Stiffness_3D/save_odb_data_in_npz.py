...
for i in range(len(jobName)):
    print(jobName[i])
    stiffMat = compaction.calculate_3D_stiffness_matrix(jobName[i])
    info = compaction.extract_info(jobName[i])
    C.append(info + [stiffMat[0][0], stiffMat[1][1], stiffMat[2][2], stiffMat[0][1], stiffMat[0][2], 
                     stiffMat[1][2], stiffMat[3][3], stiffMat[4][4], stiffMat[5][5]])
